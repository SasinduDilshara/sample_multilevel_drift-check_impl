...
resource function login(http:Caller caller, http:Request request) returns error? {
    ...
    var foundUser = from var user in userDatabase
    ...
    boolean isMatch = crypto:bcryptVerify(credentials.password.toBytes(), foundUser[0].hashedPassword);
...
function generateMockJwt(string userId) returns string {
    ...
    string header = "{\"alg\":\"HS256\",\"typ\":\"JWT\"}".toBase64();
    string payload = string `{
        "sub": "${userId}", "name": "Test User", "iat": 1516239022
    }`.toBase64();
    string signature = "mockSignature".toBase64();
